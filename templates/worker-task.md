...
Runtime policy: Orca-created child worktree; setup skipped; stateless GJC; read,search,find,edit,write only; no bash approval has been granted; run-private environment receipt recorded.
...
- Do not enable LSP. Do not rely on --no-extensions for containment. Do not use bash unless the coordinator has recorded the exact command trust gate.
...
- Send exactly one worker_done to the concrete coordinator after success, failure, or blocker. Include live taskId and dispatchId, full branch/HEAD state, changed paths, diff-stat and diff-check evidence, command exit codes, manual QA, artifact hashes, risks, and cleanup receipt.
