...
- Orca creates every task, dispatch, child worktree, and terminal. GJC never receives `--worktree`, `--tmux`, `team`, `harness`, `daemon`, `setup`, `update`, `plugin`, `completion --install`, or a global-default mutation command.
...
Every task record must include its returned task ID and dispatch ID, phase, dependencies, required flag, child worktree identity, recorded base, exclusive ownership, requested/resolved model and effort, prompt path/SHA-256, acceptance criteria, approved validations, handoff inputs/outputs, and terminal status. Accept only exact IDs returned by Orca. Filter runtime-global task listings to manifest IDs.
...
Write the worker spec as a literal run-owned file before dispatch. Its SHA-256 and byte count are the prompt-delivery input. Dispatch it only with `orca orchestration dispatch --inject`, then query `orca orchestration dispatch-show` and store the returned task/dispatch IDs and exact dispatch status. A prompt is not accepted merely because text was sent: the receipt requires the literal prompt path/hash, Orca dispatch proof, and an acknowledgment or first valid heartbeat from the recorded live worker. Prompt reinjection is allowed once only after proof that the first prompt was not accepted and a retry budget is recorded.
...
Each worker must send exactly one `worker_done`, whether successful, failed, or blocked, to the concrete coordinator handle with the live task and dispatch IDs injected by Orca. A stale dispatch ID cannot complete a retry. The payload conforms to [`schemas/completion-receipt.schema.json`](schemas/completion-receipt.schema.json), names non-empty evidence artifacts, and includes branch, full HEAD, recorded base, changed files, diff stat, exact command exit codes, manual QA, risks, and cleanup receipt. After `worker_done`, it stops and does not poll or start unrelated work.
